...
//        menuInflater.inflate(R.menu.conver_list, menu)
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.nav_private -> {
                getController()?.onListFriends()
                if (fab.isOrWillBeShown)
                    fab.hide()
            }
            R.id.nav_group_chat -> {
                getController()?.onListGroup()
                if (fab.isOrWillBeShown)
                    fab.hide()
            }
            R.id.nav_chat_room -> {
                getController()?.onListRomes()
                if (fab.isOrWillBeShown)
                    fab.hide()
            }
            R.id.nav_discussion -> {
                getController()?.onListFriendsMulti()
    ...
    fun addFabClickListener(l : View.OnClickListener) {
